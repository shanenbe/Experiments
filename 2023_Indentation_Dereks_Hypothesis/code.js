...
class IfStatement extends Statement{
    ...
    dereks_indentation_code(level) {
        return "  ".repeat(level) +
            "if (" + this.condition + ")\n"
            +  "  ".repeat(level) + "  {\n"
            + this.then_branch.dereks_indentation_code(level + 1)
            + "  ".repeat(level) + "  }\n"
            + "  ".repeat(level) + "else\n"
            +  "  ".repeat(level) +"  {\n"
            + this.else_branch.dereks_indentation_code(level + 1)
            + "  ".repeat(level) + "  }\n";
    }
...
let statements = [];
create_statements(5, statements);
...
for(let c=0; c<all_statements_with_condition.length; c++) {
    ...
    if (read_indents[""+read_indent]==undefined)
        read_indents[""+read_indent] = [];
    read_indents[""+read_indent].push(statement);
}
...
document.experiment_definition(
    {
        ...
        introduction_pages:["Thanks for participating in the experiment on indentation in source code.\n\n" +
                            "Running the experiment takes about 5 minutes.\n\n" +
        ...
        layout:[
            {variable:"Indentation",treatments:["inlinebracket", "dereksstyle"]},
        ...
        task_configuration:(t)=>{
            ...
            let possible_statements = read_indents[t.treatment_combination[1].value];
            let this_statement = possible_statements[document.new_random_integer(possible_statements.length)];

            if (t.treatment_combination[0].value=="inlinebracket") // fragt, ob die erste Variable (die einzige) den Wert "indented" hat
                t.code = this_statement.indented_code(0);
            else
                t.code = this_statement.dereks_indentation_code(0);
